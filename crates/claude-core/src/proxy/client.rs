...
pub fn build_proxy_client(config: &ProxyConfig) -> Result<Client> {
    ...
    if let Some(proxy_url) = config.active_proxy_url() {
        tracing::debug!(proxy = %proxy_url, "configuring proxy");
        let mut proxy =
    ...
    if let Some(ref ca_path) = config.ca_bundle_path {
        ...
        for cert in
            reqwest::Certificate::from_pem_bundle(&pem).with_context(|| "parsing CA bundle PEM")?
        {
