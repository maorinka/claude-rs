...
impl ToolExecutor for ListPeersTool {
    ...
    async fn call(&self, _input: &Value, _ctx: &ToolUseContext, _cancel: CancellationToken, _progress: Option<ProgressSender>) -> Result<ToolResultData> {
        let sessions_dir = {
        ...
        let mut entries = match tokio::fs::read_dir(&sessions_dir).await {
            Ok(e) => e,
            Err(_) => return Ok(ToolResultData { data: json!({ "peers": [], "count": 0 }), is_error: false }),
        };
        let my_pid = std::process::id();
        while let Ok(Some(entry)) = entries.next_entry().await {
            let file_name = entry.file_name().to_string_lossy().to_string();
            if !file_name.ends_with(".json") { continue; }
            let pid: u32 = match file_name[..file_name.len()-5].parse() { Ok(p) => p, Err(_) => continue };
            ...
            if !alive { let _ = tokio::fs::remove_file(entry.path()).await; continue; }
        ...
        let count = peers.len();
        Ok(ToolResultData { data: json!({ "peers": peers, "count": count }), is_error: false })
    }
}
...
mod tests {
    ...
    async fn list_peers_returns_empty_when_no_sessions_dir() {
        ...
        let r = ListPeersTool.call(&json!({}), &make_ctx(), CancellationToken::new(), None).await.unwrap();
        assert!(!r.is_error); assert_eq!(r.data["count"].as_u64().unwrap(), 0);
        std::env::remove_var("CLAUDE_CONFIG_DIR");
    }
    #[tokio::test]
    async fn list_peers_tool_properties() {
        assert_eq!(ListPeersTool.name(), "ListPeers"); assert!(ListPeersTool.is_read_only(&json!({}))); assert!(ListPeersTool.is_concurrency_safe(&json!({})));
    }
}
