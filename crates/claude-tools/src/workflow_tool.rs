...
impl ToolExecutor for WorkflowTool {
    ...
    fn description(&self) -> String {
        "Execute a workflow script from the .claude/workflows/ directory. \
         ...
         Use 'list' action to see available workflows, or 'run' to execute one.".to_string()
    }
    ...
    fn is_read_only(&self, input: &Value) -> bool {
        input.get("action").and_then(|v| v.as_str()).map(|a| a == "list").unwrap_or(true)
    }
        ...
        match action {
            ...
            "run" => {
                let name = match input.get("name").and_then(|v| v.as_str()) {
                    Some(n) => n,
                    None => return Ok(ToolResultData { data: json!({ "error": "missing required field: name (required for 'run' action)" }), is_error: true }),
                };
            ...
            _ => Ok(ToolResultData { data: json!({ "error": format!("Unknown action '{}'. Use 'list' or 'run'.", action) }), is_error: true }),
        }
    }
}
...
fn find_workflows_dir(working_dir: &std::path::Path) -> Option<std::path::PathBuf> {
    ...
    loop {
        ...
        if workflows.is_dir() { return Some(workflows); }
        if !dir.pop() { break; }
    }
    None
}
...
async fn list_workflows(working_dir: &std::path::Path) -> Result<ToolResultData> {
    let workflows_dir = match find_workflows_dir(working_dir) {
        Some(d) => d,
        None => return Ok(ToolResultData { data: json!({ "workflows": [], "message": "No .claude/workflows/ directory found in the project." }), is_error: false }),
    };
    let mut workflows = Vec::new();
    let mut entries = match tokio::fs::read_dir(&workflows_dir).await {
        Ok(e) => e,
        Err(e) => return Ok(ToolResultData { data: json!({ "error": format!("Failed to read workflows directory: {}", e) }), is_error: true }),
    };
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        workflows.push(json!({ "name": name, "path": path.display().to_string(), "extension": ext }));
    }
    workflows.sort_by(|a, b| a["name"].as_str().unwrap_or("").cmp(b["name"].as_str().unwrap_or("")));
    let count = workflows.len();
    Ok(ToolResultData { data: json!({ "workflows": workflows, "count": count, "directory": workflows_dir.display().to_string() }), is_error: false })
}

async fn run_workflow(working_dir: &std::path::Path, name: &str, args: &[String], cancel: CancellationToken) -> Result<ToolResultData> {
    if name.contains("..") || name.contains('/') || name.contains('\\') {
        return Ok(ToolResultData { data: json!({ "error": "Invalid workflow name: must not contain path separators or '..'" }), is_error: true });
    }
    let workflows_dir = match find_workflows_dir(working_dir) {
        Some(d) => d,
        None => return Ok(ToolResultData { data: json!({ "error": "No .claude/workflows/ directory found in the project." }), is_error: true }),
    };
    let script_path = workflows_dir.join(name);
    if !script_path.exists() {
        return Ok(ToolResultData { data: json!({ "error": format!("Workflow '{}' not found.", name) }), is_error: true });
    }
    ...
    {
        Ok(c) => c,
        Err(e) => return Ok(ToolResultData { data: json!({ "error": format!("Failed to start workflow: {}", e) }), is_error: true }),
    };
    ...
    match wait_result {
        Ok(status) => {
            ...
            let stdout_trunc = if stdout.len() > 50000 { &stdout[..50000] } else { &stdout };
            let stderr_trunc = if stderr.len() > 10000 { &stderr[..10000] } else { &stderr };
            Ok(ToolResultData {
                data: json!({ "workflow": name, "exitCode": exit_code, "stdout": stdout_trunc, "stderr": stderr_trunc, "success": status.success() }),
                is_error: !status.success(),
            })
        }
        Err(e) => Ok(ToolResultData { data: json!({ "error": format!("Workflow process error: {}", e) }), is_error: true }),
    }
}
...
mod tests {
    ...
    fn make_ctx(dir: &std::path::Path) -> ToolUseContext {
        ToolUseContext { working_directory: dir.to_path_buf(), read_file_state: Arc::new(std::sync::Mutex::new(ReadFileState::new())) }
    }
    ...
    async fn workflow_list_no_directory() {
        ...
        let result = tool.call(&json!({ "action": "list" }), &make_ctx(tmp.path()), CancellationToken::new(), None).await.unwrap();
    ...
    async fn workflow_list_with_scripts() {
        ...
        tokio::fs::write(workflows_dir.join("test-build.sh"), "#!/bin/bash\necho hello").await.unwrap();
        let tool = WorkflowTool;
        let result = tool.call(&json!({ "action": "list" }), &make_ctx(tmp.path()), CancellationToken::new(), None).await.unwrap();
        assert!(!result.is_error);
    ...
    async fn workflow_run_path_traversal_blocked() {
        let tmp = tempfile::tempdir().unwrap();
        let tool = WorkflowTool;
        let result = tool.call(&json!({ "action": "run", "name": "../etc/passwd" }), &make_ctx(tmp.path()), CancellationToken::new(), None).await.unwrap();
        assert!(result.is_error);
