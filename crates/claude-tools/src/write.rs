...
fn structured_patch_for_display(file_contents: &str, new_content: &str) -> Vec<Value> {
...
- Only use emojis if the user explicitly requests it. Avoid writing emojis to files unless asked."#.to_string()
    ...
    async fn call(
        ...
        let is_ts_update = original_file.as_deref().is_some_and(|old| !old.is_empty());
        let write_type = if is_ts_update { "update" } else { "create" };
        ...
        let structured_patch = if let Some(old_content) = original_file
            .as_deref()
            .filter(|old_content| !old_content.is_empty())
        {
            structured_patch_for_display(old_content, content)
        } else {
            Vec::new()
        };
