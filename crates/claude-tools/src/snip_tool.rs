...
impl ToolExecutor for SnipTool {
    ...
    fn description(&self) -> String {
        "Trim conversation history to free up context window space. Marks a range \
         ...
         messages are no longer relevant to the current task.".to_string()
    }
    ...
    async fn call(&self, input: &Value, _ctx: &ToolUseContext, _cancel: CancellationToken, _progress: Option<ProgressSender>) -> Result<ToolResultData> {
        let reason = match input.get("reason").and_then(|v| v.as_str()) {
            Some(r) => r,
            None => return Ok(ToolResultData { data: json!({ "error": "missing required field: reason" }), is_error: true }),
        };
        let from = input.get("messageRange").and_then(|r| r.get("from")).and_then(|v| v.as_u64());
        let to = input.get("messageRange").and_then(|r| r.get("to")).and_then(|v| v.as_u64());
        let keep_summary = input.get("keepSummary").and_then(|v| v.as_bool()).unwrap_or(true);
...
mod tests {
    ...
    async fn snip_with_reason() {
        let tool = SnipTool;
        let result = tool.call(&json!({ "reason": "Earlier debugging is no longer relevant" }), &make_ctx(), CancellationToken::new(), None).await.unwrap();
