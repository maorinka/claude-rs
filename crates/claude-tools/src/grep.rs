...
mod tests {
    ...
    #[test]
    fn test_truncate_to_max_short_string() {
        let s = "hello\nworld";
        assert_eq!(truncate_to_max(s, 100), "hello\nworld");
    }

    #[test]
    fn test_truncate_to_max_cuts_at_line_boundary() {
        let s = "line1\nline2\nline3";
        // max_chars=10 -> first 10 bytes = "line1\nline" -> rfind('\n') = 5 -> "line1\n"
        let result = truncate_to_max(s, 10);
        assert_eq!(result, "line1\n");
    }

    #[test]
    fn test_truncate_to_max_multibyte_at_boundary() {
        // Build a string where the max_chars boundary falls inside a multi-byte char
        let mut s = "a".repeat(8);
        s.push('\u{1F600}'); // 4-byte emoji at bytes [8..12]
        s.push_str("\nmore text");

        // max_chars=10 falls at byte 10, which is inside the emoji (bytes 8-11)
        // Should not panic
        let result = truncate_to_max(&s, 10);
    ...
    #[test]
    fn test_truncate_to_max_cjk_at_boundary() {
        // CJK chars are 3 bytes each
        let s = "世界和平世界和平"; // 8 CJK chars = 24 bytes
        // max_chars=7 falls inside 3rd char (bytes 6-8)
        let result = truncate_to_max(&s, 7);
        assert!(result.len() <= 7);
        assert!(result.is_char_boundary(result.len()));
    }

    #[test]
    fn test_truncate_to_max_multibyte_with_newlines() {
        // Ensure rfind('\n') works correctly after char-boundary adjustment
        let mut s = "line1\n".to_string(); // 6 bytes
        s.push('é'); // 2 bytes at [6..8]
        s.push('é'); // 2 bytes at [8..10]
        s.push_str("\nline3");

        // max_chars=9 falls inside the second 'é' (bytes 8-9)
        // After boundary adjustment, end=8, truncated="line1\né"
        // rfind('\n') at pos 5, so result = "line1\n"
        let result = truncate_to_max(&s, 9);
        assert!(result.len() <= 9);
        assert!(result.is_char_boundary(result.len()));
    }

    #[test]
    fn test_truncate_to_max_emoji_string_no_newlines() {
        // All emoji, no newlines - should truncate without rfind match
        let s: String = std::iter::repeat('\u{1F600}').take(20).collect(); // 80 bytes
        let result = truncate_to_max(&s, 10);
        assert!(result.len() <= 10);
        assert!(result.is_char_boundary(result.len()));
        // 10 / 4 = 2 full emojis = 8 bytes
