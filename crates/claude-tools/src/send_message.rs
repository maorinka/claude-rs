...
pub struct MailboxMessage {
    ...
    pub to: String,
    /// Optional human-readable summary (≤ 10 words).
...
///
/// `home_override` is used only in tests to avoid the process-global HOME env var.
...
impl ToolExecutor for SendMessageTool {
    ...
    fn description(&self) -> String {
        "Send a message to another agent via a file-based mailbox at \
         ...
         msg_<uuid>.json. The recipient polls its mailbox with receive_messages(). \
         Returns {sent, to, message_id} on success."
            .to_string()
    }
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "to": {
                    "type": "string",
                    "description": "Recipient agent ID or name."
                },
                "content": {
                    "type": "string",
                ...
                "summary": {
                    "type": "string",
                    "description": "Optional 5-10 word human-readable summary shown as a preview in the UI."
                },
                "from": {
                    "type": "string",
...
mod tests {
    ...
    // Each test calls write_to_mailbox_in / receive_messages_in with its own
    // TempDir as the home root, so tests are fully isolated and can run in
    // parallel without touching the real HOME or each other.
    ...
    // --- test send writes file to correct mailbox ---
    #[tokio::test]
    async fn test_send_writes_file_to_correct_mailbox() {
        ...
        // File should be inside the correct mailbox dir.
        ...
        // File name should follow the `msg_<id>.json` convention.
        let filename = path.file_name().unwrap().to_string_lossy().to_string();
        assert_eq!(filename, "msg_test-id-1.json");

        // Content should round-trip as MailboxMessage.
    ...
    // --- test receive reads and deletes messages ---
    #[tokio::test]
    async fn test_receive_reads_and_deletes_messages() {
        ...
        // Messages should be in sorted (alphabetical → chronological) order.
        let ids: Vec<_> = received.iter().map(|m| m.id.as_str()).collect();
        assert_eq!(ids, vec!["recv-id-0", "recv-id-1"]);
    }

    // --- test send to nonexistent agent creates mailbox ---
    #[tokio::test]
    async fn test_send_to_nonexistent_agent_creates_mailbox() {
        ...
        // Mailbox directory must NOT exist yet.
        let dir = mailbox_dir_with_home(&home, agent_id);
    ...
    // --- test receive on missing mailbox returns empty vec ---
    #[tokio::test]
    ...
    // --- test ToolExecutor::call returns sent=true + message_id ---
    #[tokio::test]
    async fn test_tool_call_returns_sent_true() {
        use crate::registry::ReadFileState;
        use std::sync::{Arc, Mutex};

        // The real `call()` uses the global HOME; point it to a temp dir.
        let tmp = TempDir::new().unwrap();
        // SAFETY: tests run in separate OS threads / processes — env var racing
        // is acceptable for this integration-style test.
        ...
        let home = tmp.path().to_path_buf();
        let agent_dir = mailbox_dir_with_home(&home, "tool-target");
        let file_count = std::fs::read_dir(&agent_dir).unwrap().count();
        assert_eq!(file_count, 1, "expected exactly one message file in the mailbox");
    }

    // --- test ToolExecutor::call returns error on missing `to` ---
    #[tokio::test]
