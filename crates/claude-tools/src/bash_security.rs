...
fn validate_incomplete_commands(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    if ctx.original_command.starts_with('\t') || ctx.original_command.starts_with(" \t") {
        return SecurityBehavior::Ask("Command appears to be an incomplete fragment (starts with tab)".into());
    }
    if trimmed.starts_with('-') {
        return SecurityBehavior::Ask("Command appears to be an incomplete fragment (starts with flags)".into());
    }
    if trimmed.starts_with("&&") || trimmed.starts_with("||") || trimmed.starts_with(';')
        || trimmed.starts_with(">>") || trimmed.starts_with('>') || trimmed.starts_with('<')
    {
        return SecurityBehavior::Ask("Command appears to be a continuation line (starts with operator)".into());
    }
    SecurityBehavior::Passthrough
}
...
fn validate_jq_command(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    if ctx.original_command.contains("system") && ctx.original_command.contains('(') {
        return SecurityBehavior::Ask("jq command contains system() function which executes arbitrary commands".into());
    }
    let after_jq = if ctx.original_command.len() > 3 { ctx.original_command[3..].trim() } else { "" };
    for flag in &["-f", "--from-file", "--rawfile", "--slurpfile", "-L", "--library-path"] {
        if after_jq.contains(flag) {
            return SecurityBehavior::Ask("jq command contains dangerous flags that could read arbitrary files".into());
        }
    }
    SecurityBehavior::Passthrough
}
...
fn validate_obfuscated_flags(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    if regex_lite::Regex::new(r"\$'[^']*'").unwrap().is_match(cmd) {
        return SecurityBehavior::Ask("Command contains ANSI-C quoting which can hide characters".into());
    }
    if regex_lite::Regex::new(r#"\$"[^"]*""#).unwrap().is_match(cmd) {
        return SecurityBehavior::Ask("Command contains locale quoting which can hide characters".into());
    }
    if regex_lite::Regex::new(r#"\$['"]['"]\s*-"#).unwrap().is_match(cmd) {
        return SecurityBehavior::Ask("Command contains empty special quotes before dash".into());
    }
    if regex_lite::Regex::new(r#"(?:^|\s)(?:''|"")+\s*-"#).unwrap().is_match(cmd) {
        return SecurityBehavior::Ask("Command contains empty quotes before dash".into());
    }
    if regex_lite::Regex::new(r#"(?:""|'')+['"]-"#).unwrap().is_match(cmd) {
        return SecurityBehavior::Ask("Command contains empty quote pair adjacent to quoted dash".into());
    }
    if regex_lite::Regex::new(r#"(?:^|\s)['\"]{3,}"#).unwrap().is_match(cmd) {
        return SecurityBehavior::Ask("Command contains consecutive quote characters at word start".into());
    }
    if regex_lite::Regex::new(r#"\s['"`]-"#).unwrap().is_match(&ctx.fully_unquoted_content) {
        return SecurityBehavior::Ask("Command contains quoted characters in flag names".into());
    }
    if regex_lite::Regex::new(r#"['"`]{2}-"#).unwrap().is_match(&ctx.fully_unquoted_content) {
        return SecurityBehavior::Ask("Command contains quoted characters in flag names".into());
    }
    SecurityBehavior::Passthrough
}
...
fn validate_shell_metacharacters(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    if regex_lite::Regex::new(r#"(?:^|\s)["'][^"']*[;&][^"']*["'](?:\s|$)"#).unwrap().is_match(content) {
...
fn validate_dangerous_variables(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    if regex_lite::Regex::new(r"[<>|]\s*\$[A-Za-z_]").unwrap().is_match(content)
        || regex_lite::Regex::new(r"\$[A-Za-z_][A-Za-z0-9_]*\s*[|<>]").unwrap().is_match(content)
    {
...
fn validate_newlines(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    if !content.contains('\n') && !content.contains('\r') { return SecurityBehavior::Passthrough; }
    let bytes = content.as_bytes();
    for i in 0..bytes.len() {
        if bytes[i] == b'\n' || bytes[i] == b'\r' {
            ...
            if j < bytes.len() && !matches!(bytes[j], b'\n' | b'\r') {
                return SecurityBehavior::Ask("Command contains newlines that could separate multiple commands".into());
            }
        }
    }
    SecurityBehavior::Passthrough
...
fn validate_carriage_return(ctx: &ValidationContext) -> SecurityBehavior {
    let cmd = &ctx.original_command;
    if !cmd.contains('\r') { return SecurityBehavior::Passthrough; }
    let mut in_sq = false;
    let mut in_dq = false;
    let mut escaped = false;
    for ch in cmd.chars() {
        if escaped { escaped = false; continue; }
        if ch == '\\' && !in_sq { escaped = true; continue; }
        if ch == '\'' && !in_dq { in_sq = !in_sq; continue; }
        if ch == '"' && !in_sq { in_dq = !in_dq; continue; }
        if ch == '\r' && !in_dq {
            return SecurityBehavior::Ask("Command contains carriage return causing tokenization differential".into());
        }
    }
    SecurityBehavior::Passthrough
}
...
fn validate_ifs_injection(ctx: &ValidationContext) -> SecurityBehavior {
    let cmd = &ctx.original_command;
    if cmd.contains("$IFS") || regex_lite::Regex::new(r"\$\{[^}]*IFS").unwrap().is_match(cmd) {
...
fn validate_proc_environ_access(ctx: &ValidationContext) -> SecurityBehavior {
    if regex_lite::Regex::new(r"/proc/.*/environ").unwrap().is_match(&ctx.original_command) {
    ...
    while i < bytes.len() {
        if bytes[i] == b'\\' && !in_sq {
            if !in_dq && i + 1 < bytes.len() && matches!(bytes[i + 1], b' ' | b'\t') {
                return SecurityBehavior::Ask("Command contains backslash-escaped whitespace".into());
            }
            i += 2; continue;
        }
        if bytes[i] == b'"' && !in_sq { in_dq = !in_dq; }
        else if bytes[i] == b'\'' && !in_dq { in_sq = !in_sq; }
        i += 1;
    }
    SecurityBehavior::Passthrough
}
    ...
    while i < bytes.len() {
        if bytes[i] == b'\\' && !in_sq {
            if !in_dq && i + 1 < bytes.len() && matches!(bytes[i + 1], b';' | b'|' | b'&' | b'<' | b'>') {
                return SecurityBehavior::Ask("Command contains backslash before a shell operator".into());
            }
            i += 2; continue;
        }
        if bytes[i] == b'\'' && !in_dq { in_sq = !in_sq; }
        else if bytes[i] == b'"' && !in_sq { in_dq = !in_dq; }
        i += 1;
    }
    SecurityBehavior::Passthrough
}
...
fn validate_mid_word_hash(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    for i in 1..bytes.len() {
        if bytes[i] == b'#' && !matches!(bytes[i - 1], b' ' | b'\t' | b'\n' | b'\r') {
            if i >= 2 && bytes[i - 2] == b'$' && bytes[i - 1] == b'{' { continue; }
            return SecurityBehavior::Ask("Command contains mid-word # (parser differential)".into());
        }
    }
    SecurityBehavior::Passthrough
}
...
fn is_escaped_at_position(bytes: &[u8], pos: usize) -> bool {
    let mut count = 0u32;
    let mut i = pos;
    while i > 0 { i -= 1; if bytes[i] == b'\\' { count += 1; } else { break; } }
    count % 2 == 1
}
...
fn validate_brace_expansion(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    for i in 0..bytes.len() {
        if bytes[i] == b'{' && !is_escaped_at_position(bytes, i) { open += 1; }
        else if bytes[i] == b'}' && !is_escaped_at_position(bytes, i) { close += 1; }
    }
    if open > 0 && close > open {
        return SecurityBehavior::Ask("Command has excess closing braces (brace expansion obfuscation)".into());
    }
    if open > 0 && regex_lite::Regex::new(r#"['"][{}]['"]"#).unwrap().is_match(&ctx.original_command) {
        return SecurityBehavior::Ask("Command contains quoted brace inside brace context".into());
    }
    let mut i = 0;
    while i < bytes.len() {
        if bytes[i] != b'{' || is_escaped_at_position(bytes, i) { i += 1; continue; }
        let mut depth: i32 = 1;
        let mut mc: Option<usize> = None;
        let mut j = i + 1;
        while j < bytes.len() {
            if bytes[j] == b'{' && !is_escaped_at_position(bytes, j) { depth += 1; }
            else if bytes[j] == b'}' && !is_escaped_at_position(bytes, j) { depth -= 1; if depth == 0 { mc = Some(j); break; } }
            j += 1;
        }
        if let Some(cp) = mc {
            let mut id: i32 = 0;
            let mut k = i + 1;
            while k < cp {
                if bytes[k] == b'{' && !is_escaped_at_position(bytes, k) { id += 1; }
                else if bytes[k] == b'}' && !is_escaped_at_position(bytes, k) { id -= 1; }
                else if id == 0 && (bytes[k] == b',' || (bytes[k] == b'.' && k + 1 < cp && bytes[k + 1] == b'.')) {
...
fn validate_comment_quote_desync(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    while i < chars.len() {
        ...
        if in_sq { if ch == '\'' { in_sq = false; } i += 1; continue; }
        if ch == '\\' { escaped = true; i += 1; continue; }
        if in_dq { if ch == '"' { in_dq = false; } i += 1; continue; }
        ...
        if ch == '#' {
            ...
            while j < chars.len() && chars[j] != '\n' {
                if chars[j] == '\'' || chars[j] == '"' {
                    return SecurityBehavior::Ask("Command contains quotes inside # comment (desync)".into());
                }
                j += 1;
            }
            i = j; continue;
...
fn validate_quoted_newline(ctx: &ValidationContext) -> SecurityBehavior {
    let cmd = &ctx.original_command;
    if !cmd.contains('\n') || !cmd.contains('#') { return SecurityBehavior::Passthrough; }
    let mut in_sq = false;
    let mut in_dq = false;
    let mut escaped = false;
    let bytes = cmd.as_bytes();
    for i in 0..bytes.len() {
        let ch = bytes[i];
        if escaped { escaped = false; continue; }
        if ch == b'\\' && !in_sq { escaped = true; continue; }
        if ch == b'\'' && !in_dq { in_sq = !in_sq; continue; }
        if ch == b'"' && !in_sq { in_dq = !in_dq; continue; }
        if ch == b'\n' && (in_sq || in_dq) {
            let rest = &cmd[i + 1..];
            if let Some(line) = rest.lines().next() {
                if line.trim().starts_with('#') {
                    return SecurityBehavior::Ask("Command contains quoted newline followed by #-prefixed line".into());
                }
            }
        }
    }
...
const ZSH_DANGEROUS_COMMANDS: &[&str] = &[
    ...
    "zsocket", "mapfile", "zf_rm", "zf_mv", "zf_ln", "zf_chmod", "zf_chown", "zf_mkdir",
    "zf_rmdir", "zf_chgrp",
];
...
fn validate_zsh_dangerous_commands(ctx: &ValidationContext) -> SecurityBehavior {
    ...
    let mut base_cmd = "";
    for token in &tokens {
        if env_re.is_match(token) || zsh_mods.contains(token) { continue; }
        base_cmd = token; break;
    }
    if ZSH_DANGEROUS_COMMANDS.contains(&base_cmd) {
        return SecurityBehavior::Ask(format!("Command uses Zsh-specific '{}'", base_cmd));
    }
    if base_cmd == "fc" && regex_lite::Regex::new(r"\s-\S*e").unwrap().is_match(trimmed) {
...
pub fn validate_command_security(command: &str) -> SecurityBehavior {
    ...
    let misparsing: Vec<fn(&ValidationContext) -> SecurityBehavior> = vec![
        ...
        validate_carriage_return, validate_ifs_injection, validate_proc_environ_access,
    ...
    for v in &misparsing {
        let r = v(&ctx);
        if let SecurityBehavior::Ask(_) = &r { return r; }
    }

    // Non-misparsing (deferred)
    let non_misp: Vec<fn(&ValidationContext) -> SecurityBehavior> = vec![validate_newlines, validate_redirections];
    let mut deferred: Option<SecurityBehavior> = None;
    for v in &non_misp {
        let r = v(&ctx);
        if let SecurityBehavior::Ask(_) = &r { if deferred.is_none() { deferred = Some(r); } }
    }
    if let Some(d) = deferred { return d; }
...
pub const EXPANDED_READ_ONLY_COMMANDS: &[&str] = &[
    "ls", "cat", "head", "tail", "less", "more", "wc", "file", "stat", "readlink", "realpath",
    "du", "df", "lsof", "lsblk", "lscpu", "find", "grep", "egrep", "fgrep", "rg", "ag", "ack",
    "which", "where", "type", "whence", "whereis", "locate", "echo", "printf", "date", "pwd",
    "whoami", "hostname", "uname", "env", "printenv", "id", "groups", "true", "false", "test",
    "[", "git status", "git log", "git diff", "git show", "git branch", "git remote", "git tag",
    "git stash list", "git rev-parse", "git ls-files", "git ls-remote", "git ls-tree",
    "git describe", "git blame", "git shortlog", "git name-rev", "git rev-list",
    "git for-each-ref", "git count-objects", "git verify-commit", "git verify-tag",
    "git config --get", "git config --list", "git cat-file", "cargo check", "cargo test",
    "cargo clippy", "cargo build", "cargo doc", "cargo bench", "cargo metadata", "cargo tree",
    "npm test", "npm run lint", "npm run build", "npm run check", "npm ls", "npm list",
    "npm info", "npm view", "npm outdated", "npx tsc", "node -e", "node -p", "python -c",
    "python3 -c", "python -m py_compile", "pip list", "pip show", "pip freeze", "pip3 list",
    "pip3 show", "pip3 freeze", "gem list", "go list", "go vet", "go version", "rustup show",
    "rustc --version", "rustfmt --check", "docker ps", "docker images", "docker inspect",
    "docker logs", "docker version", "docker info", "docker stats", "docker-compose ps",
    "docker-compose logs", "uptime", "free", "vmstat", "ps", "pgrep", "nproc", "arch", "getconf",
    "dig", "host", "nslookup", "sort", "uniq", "tr", "cut", "paste", "column", "fmt", "fold",
    "expand", "unexpand", "nl", "diff", "comm", "cmp", "md5sum", "sha256sum", "sha1sum",
    "base64", "xxd", "od", "hexdump", "jq", "gh pr list", "gh pr view", "gh pr status",
    "gh issue list", "gh issue view", "gh issue status", "gh repo view", "gh api", "gh run list",
    "gh run view", "man", "help", "info", "cal", "bc", "basename", "dirname", "seq", "sleep",
    "tput", "clear", "reset", "xdg-open", "open",
];

const DESTRUCTIVE_COMMANDS: &[&str] = &[
    "rm", "rmdir", "mv", "cp", "chmod", "chown", "chgrp", "truncate", "shred", "mkdir",
    "touch", "ln", "kill", "killall", "pkill", "reboot", "shutdown", "halt", "poweroff",
    "mount", "umount", "useradd", "userdel", "usermod", "groupadd", "groupdel", "passwd",
    "chpasswd", "apt", "apt-get", "dpkg", "yum", "dnf", "pacman", "brew", "pip install",
    "pip3 install", "pip uninstall", "pip3 uninstall", "npm install", "npm uninstall",
    "npm update", "npm publish", "cargo install", "gem install", "gem uninstall", "curl",
    "wget", "sudo", "su", "doas", "git push", "git commit", "git add", "git rm", "git mv",
    "git reset", "git checkout", "git rebase", "git merge", "git cherry-pick", "git revert",
    "git clean", "git stash drop", "git stash pop", "git stash clear", "git branch -d",
    "git branch -D", "git push --force", "git push -f", "docker run", "docker exec",
    "docker build", "docker pull", "docker push", "docker rm", "docker rmi", "docker stop",
    "docker kill", "docker restart", "docker compose up", "dd", "mkfs", "fdisk", "parted",
    "iptables", "ip6tables", "nft", "systemctl", "service", "crontab",
];
...
pub fn strip_safe_wrappers(command: &str) -> String {
    let env_var_re = regex_lite::Regex::new(r"^([A-Za-z_][A-Za-z0-9_]*)=([A-Za-z0-9_./:-]+)\s+").unwrap();
    ...
    while stripped != prev {
        ...
        if !lines.is_empty() { stripped = lines.join("\n"); }
        if let Some(m) = env_var_re.find(&stripped) {
            let vn = stripped[..m.end()].split('=').next().unwrap_or("");
            if SAFE_ENV_VARS.contains(&vn) { stripped = stripped[m.end()..].to_string(); }
        }
    }
    prev = String::new();
    while stripped != prev {
        ...
        for pat in &wrapper_patterns {
            if let Some(m) = pat.find(&stripped) { stripped = stripped[m.end()..].to_string(); break; }
        }
    }
    stripped
}
...
pub fn is_command_read_only(command: &str) -> bool {
    ...
    EXPANDED_READ_ONLY_COMMANDS.iter().any(|safe| cmd == *safe || cmd.starts_with(&format!("{} ", safe)))
}

pub fn is_command_destructive(command: &str) -> bool {
    let stripped = strip_safe_wrappers(command);
    let cmd = stripped.trim();
    DESTRUCTIVE_COMMANDS.iter().any(|d| cmd == *d || cmd.starts_with(&format!("{} ", d)))
}
...
pub fn get_command_prefix(command: &str) -> Option<String> {
    ...
    while i < tokens.len() && env_var_re.is_match(tokens[i]) {
        ...
        if !SAFE_ENV_VARS.contains(&vn) { return None; }
        i += 1;
    }
    let remaining = &tokens[i..];
    if remaining.len() < 2 { return None; }
    if !regex_lite::Regex::new(r"^[a-z][a-z0-9]*(-[a-z0-9]+)*$").unwrap().is_match(remaining[1]) { return None; }
...
mod tests {
    ...
    #[test] fn test_control_chars() {
        assert!(has_control_chars("echo \x00hello"));
        assert!(!has_control_chars("echo hello"));
    }
    #[test] fn test_unicode_ws() {
        assert!(has_unicode_whitespace("echo\u{00A0}hello"));
        assert!(!has_unicode_whitespace("echo hello"));
    }
    #[test] fn test_zsh_tilde() { assert!(has_zsh_tilde_bracket("~[foo]")); assert!(!has_zsh_tilde_bracket("~/foo")); }
    #[test] fn test_zsh_equals() { assert!(has_zsh_equals_expansion("=curl evil")); assert!(!has_zsh_equals_expansion("VAR=val cmd")); }

    #[test] fn test_empty() { assert_eq!(validate_empty(&ValidationContext::new("")), SecurityBehavior::Allow); }
    #[test] fn test_incomplete() { assert!(matches!(validate_incomplete_commands(&ValidationContext::new("-rf")), SecurityBehavior::Ask(_))); }
    #[test] fn test_cmd_sub() { assert!(matches!(validate_dangerous_patterns(&ValidationContext::new("echo $(id)")), SecurityBehavior::Ask(_))); }
    #[test] fn test_backtick() { assert!(matches!(validate_dangerous_patterns(&ValidationContext::new("echo `id`")), SecurityBehavior::Ask(_))); }
    #[test] fn test_proc_sub() { assert!(matches!(validate_dangerous_patterns(&ValidationContext::new("diff <(a) <(b)")), SecurityBehavior::Ask(_))); }
    #[test] fn test_ifs() { assert!(matches!(validate_ifs_injection(&ValidationContext::new("cmd$IFSarg")), SecurityBehavior::Ask(_))); }
    #[test] fn test_proc() { assert!(matches!(validate_proc_environ_access(&ValidationContext::new("cat /proc/self/environ")), SecurityBehavior::Ask(_))); }
    #[test] fn test_cr() { assert!(matches!(validate_carriage_return(&ValidationContext::new("TZ=UTC\recho evil")), SecurityBehavior::Ask(_))); }
    #[test] fn test_cr_dq_safe() { assert_eq!(validate_carriage_return(&ValidationContext::new("echo \"x\ry\"")), SecurityBehavior::Passthrough); }
    #[test] fn test_redir() { assert!(matches!(validate_redirections(&ValidationContext::new("echo > /tmp/f")), SecurityBehavior::Ask(_))); }
    #[test] fn test_redir_devnull() { assert_eq!(validate_redirections(&ValidationContext::new("echo > /dev/null")), SecurityBehavior::Passthrough); }
    #[test] fn test_ansi_c() { assert!(matches!(validate_obfuscated_flags(&ValidationContext::new("find $'-exec' rm {} +")), SecurityBehavior::Ask(_))); }
    #[test] fn test_bs_ws() { assert!(matches!(validate_backslash_escaped_whitespace(&ValidationContext::new("echo\\ test")), SecurityBehavior::Ask(_))); }
    #[test] fn test_bs_op() { assert!(matches!(validate_backslash_escaped_operators(&ValidationContext::new("cat f \\; echo /etc/passwd")), SecurityBehavior::Ask(_))); }
    #[test] fn test_brace_comma() { assert!(matches!(validate_brace_expansion(&ValidationContext::new("echo {a,b}")), SecurityBehavior::Ask(_))); }
    #[test] fn test_brace_seq() { assert!(matches!(validate_brace_expansion(&ValidationContext::new("echo {1..10}")), SecurityBehavior::Ask(_))); }
    #[test] fn test_zsh_zmodload() { assert!(matches!(validate_zsh_dangerous_commands(&ValidationContext::new("zmodload zsh/system")), SecurityBehavior::Ask(_))); }
    #[test] fn test_fc_e() { assert!(matches!(validate_zsh_dangerous_commands(&ValidationContext::new("fc -e vim")), SecurityBehavior::Ask(_))); }
    #[test] fn test_jq_system() { assert!(matches!(validate_jq_command(&ValidationContext::new("jq 'system(\"rm\")'")), SecurityBehavior::Ask(_))); }
    #[test] fn test_jq_flag() { assert!(matches!(validate_jq_command(&ValidationContext::new("jq -f evil.jq in")), SecurityBehavior::Ask(_))); }

    #[test] fn test_eval_like() { assert!(check_eval_like_builtins("eval")); assert!(!check_eval_like_builtins("echo")); }
    #[test] fn test_read_only() { assert!(is_command_read_only("ls -la")); assert!(is_command_read_only("git status")); assert!(!is_command_read_only("rm -rf /")); }
    #[test] fn test_destructive() { assert!(is_command_destructive("rm -rf /")); assert!(!is_command_destructive("ls")); }
    #[test] fn test_wc_exact() { assert!(match_wildcard_pattern("git status", "git status")); assert!(!match_wildcard_pattern("git status", "git status -v")); }
    #[test] fn test_wc_prefix() { assert!(match_wildcard_pattern("npm run:*", "npm run build")); assert!(!match_wildcard_pattern("npm run:*", "npm install")); }
    #[test] fn test_strip_nohup() { assert_eq!(strip_safe_wrappers("nohup git push"), "git push"); }
    #[test] fn test_strip_env() { assert_eq!(strip_safe_wrappers("NODE_ENV=prod npm run build"), "npm run build"); }
