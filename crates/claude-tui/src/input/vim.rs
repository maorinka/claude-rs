...
impl VimBuffer {
    ...
    pub fn move_right(&mut self) {
        ...
        if self.cursor < len {
            let mut n = self.cursor + 1;
            while n < len && !self.text.is_char_boundary(n) {
                n += 1;
            }
    ...
    pub fn next_word(&mut self) {
        ...
        while i < len && !is_word_byte(bytes[i]) && !bytes[i].is_ascii_whitespace() {
            i += 1;
        }
    ...
    pub fn prev_word(&mut self) {
        ...
        if i > 0 && !is_word_byte(bytes[i]) {
            while i > 0 && !is_word_byte(bytes[i]) && !bytes[i].is_ascii_whitespace() {
                i -= 1;
            }
            if is_word_byte(bytes[i]) || bytes[i].is_ascii_whitespace() {
                i += 1;
            }
        } else {
            while i > 0 && is_word_byte(bytes[i.saturating_sub(1)]) {
                i -= 1;
            }
        }
        self.cursor = i;
    ...
    pub fn end_of_word(&mut self) {
        ...
        if i < len && is_word_byte(bytes[i]) {
            while i + 1 < len && is_word_byte(bytes[i + 1]) {
                i += 1;
            }
        } else {
            while i + 1 < len && !is_word_byte(bytes[i + 1]) && !bytes[i + 1].is_ascii_whitespace()
            {
                i += 1;
            }
        }
            ...
            FindType::ForwardTo | FindType::ForwardBefore => {
                ...
                for (i, c) in self.text[start..].char_indices() {
                    if c == ch {
                        ...
                        if found == count {
                            let pos = start + i;
                            return Some(if find_type == FindType::ForwardBefore {
                                pos.saturating_sub(1).max(self.cursor)
                            } else {
                                pos
                            });
                        }
            ...
            FindType::BackwardTo | FindType::BackwardAfter => {
                ...
                for (i, c) in self.text[..self.cursor].char_indices().rev() {
                    if c == ch {
                        found += 1;
                        if found == count {
                            return Some(if find_type == FindType::BackwardAfter {
                                (i + c.len_utf8()).min(self.cursor)
                            } else {
                                i
                            });
                        }
...
fn find_word_object(
    ...
    offset: usize,
    is_inner: bool,
    word_chars_only: bool,
) -> Option<(usize, usize)> {
    let bytes = text.as_bytes();
    if offset >= bytes.len() {
        return None;
    }
    let is_wc: fn(u8) -> bool = if word_chars_only {
        is_word_byte
    } else {
    ...
    if in_word {
        while s > 0 && is_wc(bytes[s - 1]) {
            s -= 1;
        }
        while e < bytes.len() && is_wc(bytes[e]) {
            e += 1;
        }
    } else {
        let ip = |b: u8| !is_word_byte(b) && !b.is_ascii_whitespace();
        while s > 0 && ip(bytes[s - 1]) {
            s -= 1;
        }
        while e < bytes.len() && ip(bytes[e]) {
            e += 1;
        }
    }
    if !is_inner {
...
fn find_quote_object(
    ...
    for pair in positions.chunks(2) {
        if pair.len() == 2 && pair[0] <= pos && pos <= pair[1] {
            return if is_inner {
                Some((ls + pair[0] + 1, ls + pair[1]))
            } else {
                Some((ls + pair[0], ls + pair[1] + 1))
            };
        }
    }
    None
...
// Motions
#[allow(dead_code)]
fn is_inclusive_motion(key: &str) -> bool {
    matches!(key, "e" | "E" | "$")
}
#[allow(dead_code)]
    ...
    for _ in 0..count {
        pos = match key {
            ...
            "B" => {
                ...
                while i > 0 && !bytes[i.saturating_sub(1)].is_ascii_whitespace() {
                    i -= 1;
                }
                i
            },
...
fn apply_operator(
    ...
    persistent.register_is_linewise = linewise;
    match op {
        Operator::Yank => {
            buf.cursor = from;
            OperatorEffect::None
